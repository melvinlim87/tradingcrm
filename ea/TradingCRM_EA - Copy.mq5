...
input string  InpEaToken      = "REPLACE_WITH_EA_PUSH_TOKEN";    // Matches .env EA_PUSH_TOKEN
...
{
   Print("==================================================");
   Print("  TradingCRM EA v3.0 (combined)");
   Print("==================================================");
   PrintFormat("  Backend: %s", InpBackendBase);
   PrintFormat("  Timers — Signal:%ds  RiskPush:%ds  ChartPoll:%ds",
...
void ProcessChartRequest(int requestId, string symbol)
{
   ...
   for(int i = 0; i < ArraySize(TIMEFRAMES); i++)
   {
      ...
      Sleep(500);
      UploadChartScreenshot(requestId, symbol, tfName, filename);
      ChartClose(chartId);
   }
}
